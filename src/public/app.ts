...
function showModalAlert(message: string): void {
  const alert = document.getElementById('modal-alert') as HTMLElement;
  alert.innerText = message;
  alert.classList.remove('d-none');
}
...
async function openEditModal(id: number): Promise<void> {
  try {
    ...
    (document.getElementById('modal-title') as HTMLInputElement).value = todo.title;
    (document.getElementById('modal-description') as HTMLTextAreaElement).value = todo.description;
    (document.getElementById('modal-completed') as HTMLInputElement).checked = todo.completed;
...
async function saveEdit(): Promise<void> {
  ...
  const completed = (document.getElementById('modal-completed') as HTMLInputElement).checked;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  (window as any).$('#modal').on('hidden.bs.modal', () => {
    hideModalAlert();
    editingId = null;
  });
